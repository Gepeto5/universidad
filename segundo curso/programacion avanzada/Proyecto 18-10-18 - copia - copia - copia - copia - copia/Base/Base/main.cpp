...
//clase abstracta pura suele caer en examenes
...
void init(void) {
	...
	Triangulo* t = nullptr;
	t = new Triangulo(Vector3D(0, 0, 3), Vector3D(0, 5, 3), Vector3D(5, 0, 3));
	e.add(t);
...
Vector3D vecsToString(string line) {
	string st;
	double x, y, z;
	istringstream ss(line); //coge un string y lo transforma en fichero
	ss >> st >> x >> y >> z;
...
int main(int argc, char** argv) {
	...
	ofstream of("example.txt");
	if (of.is_open()) {
		of << "¿Huele Adrián a culo?" << endl;
		of << "Sí" << endl;
		of.close();
	}
	else {
	...
	*/
	...
	if (model.is_open()) {
		while (getline(model, line)) {
			//vecmodel.push_back(line);
			//cout << vecmodel.back() << "\n";
			if (line.size() > 2) {
				if (line[0] == 'v' && line[1] == ' ') {
					verts.push_back(vecsToString(line));
					cout << verts.back() << "\n";
				}
				else if (line[0] == 'f') {
